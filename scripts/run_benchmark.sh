...
if [ "${QUERY_PQ_BYTES}" != "0" ]; then
  INDEX_QUERY_PQ_SUFFIX="_QPQ${QUERY_PQ_BYTES}"
fi
INDEX_EXPERIMENT="${INDEX_EXPERIMENT:-${PREFIX}_starling}"
INDEX_NAME="${INDEX_NAME:-${PREFIX}_R${R}_L${BUILD_L}_B${B}_M${M}${INDEX_QUERY_PQ_SUFFIX}${INDEX_DISK_PQ_SUFFIX}}"
INDEX_DIR="${STARLING_INDEX_ROOT}/${INDEX_EXPERIMENT}/${INDEX_NAME}"
INDEX_PREFIX_PATH="${INDEX_DIR}/${INDEX_NAME}"
REPORT_EXPERIMENT="${REPORT_EXPERIMENT:-${INDEX_EXPERIMENT}}"
REPORT_NAME="${REPORT_NAME:-${INDEX_NAME}}"
REPORT_DIR="${STARLING_REPORT_ROOT}/${REPORT_EXPERIMENT}/${REPORT_NAME}"
...
case $2 in
  build)
    ...
    time ${EXE_PATH}/tests/build_disk_index \
      ...
      --PQ_search_bytes "${QUERY_PQ_BYTES}" \
